...
namespace UnrealSharp
{
    public unsafe class Memory
    {
        ...
        [DllImport("kernel32", EntryPoint = "VirtualQuery")] public static extern IntPtr VirtualQuery(nint lpAddress, out MEMORY_BASIC_INFORMATION64 lpBuffer, UIntPtr dwLength);
        public int ReadProcessMemory2(nint handle, nint addr, byte[] buf, int size, out int bytesRead)
        {
            bytesRead = size;
            var mbi = new MEMORY_BASIC_INFORMATION64();
            var retVal = VirtualQuery(addr, out mbi, new UIntPtr((uint)Marshal.SizeOf(mbi)));
            if (mbi.State != 0x1000) return 0;
            //System.Buffer.MemoryCopy((void*)addr, (void*)buf, size, size);
            Marshal.Copy(addr, buf, 0, size);
            return 0;
        }
        public Boolean WriteProcessMemory(nint hProcess, nint lpBaseAddress, Byte[] buffer, Int32 nSize, out Int32 lpNumberOfBytesWritten)
        {
            Marshal.Copy(buffer, 0, lpBaseAddress, nSize);
